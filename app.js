...
function openDrawer() {
  ...
  if (exists("settingsBackdrop")) $("settingsBackdrop").hidden = false;
...
function closeDrawer() {
  ...
  setTimeout(() => {
    if (!$('settingsBackdrop')?.classList.contains('open') && exists('settingsBackdrop')) $('settingsBackdrop').hidden = true;
  }, 260);
}
...
function decodeBase64Utf8(text) { return decodeURIComponent(escape(atob(text.replace(/\n/g, "")))); }

function githubHeaders() {
  return { "Accept": "application/vnd.github+json", "Authorization": `Bearer ${getGithubToken()}`, "X-GitHub-Api-Version": "2022-11-28" };
}
...
function syncToGithub() {
  if (!getGithubToken()) { setSyncStatus("Local"); return; }
...
async function pushGithubData() {
  ...
  try {
    ...
    const payload = { updatedAt: new Date().toISOString(), app: "Yair OS", source: "GitHub Pages", data: state };
    const body = { message: `Update Yair OS data ${new Date().toISOString()}`, content: encodeBase64Utf8(JSON.stringify(payload, null, 2)), branch: GITHUB_BRANCH };
    ...
    const response = await fetch(url, { method: "PUT", headers: { ...githubHeaders(), "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (!response.ok) throw new Error(`GitHub respondió ${response.status}`);
    ...
    if (syncQueued) { syncQueued = false; syncToGithub(); }
  }
}

async function pullGithubData() {
  if (!getGithubToken()) { alert("Primero pega tu GitHub token y presiona Activar sync GitHub."); return; }
  setSyncStatus("Leyendo...");
  try {
    const currentFile = await getGithubFile();
    if (!currentFile?.content) { setSyncStatus("Sin archivo"); return; }
...
async function sendToGoogle(action, payload) {
  ...
  try {
    const response = await fetch(scriptUrl, { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ action, payload, app: "Yair OS", createdAt: new Date().toISOString() }) });
...
async function askOpenAi() {
  ...
  if (!answer) return;
  if (!proxyUrl) { answer.className = "ai-answer empty"; answer.textContent = "Primero configura el OpenAI proxy URL."; return; }
  if (!prompt) { answer.className = "ai-answer empty"; answer.textContent = "Escribe una pregunta para tu sistema."; return; }
  answer.className = "ai-answer";
  answer.textContent = "Analizando...";
  const context = { transactions: state.transactions.slice(-50), tasks: state.tasks.slice(-30), memories: state.memories.slice(-30), totals: getTotals() };
  try {
    const response = await fetch(proxyUrl, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt, context }) });
...
async function addTransaction() {
  ...
  const transaction = { id: safeUUID(), type: selectedType, amount, description, category, paymentMethod, date: new Date().toISOString() };
...
async function addMemory(event) {
  ...
  if (!text) return;
  const memory = { id: safeUUID(), date: new Date().toISOString(), category: $("memoryCategory")?.value || "Personal", text, priority: "Media", status: "Activo", source: "Yair OS App" };
...
async function addTask(event) {
  ...
  if (!text) return;
  const task = { id: safeUUID(), text, area: $("taskArea")?.value || "Personal", targetDate: $("taskDate")?.value || "", done: false, date: new Date().toISOString() };
...
function renderTasks() {
  const list = $("taskList");
  if (!list) return;
  if (!state.tasks.length) { list.className = "transaction-list empty"; list.textContent = "Sin pendientes todavía."; return; }
  list.className = "transaction-list";
  list.innerHTML = state.tasks.slice().reverse().map((t) => `<div class="transaction-row"><div><strong>${t.done ? "✓ " : ""}${escapeHtml(t.text)}</strong><small>${escapeHtml(t.area)}${t.targetDate ? " · " + escapeHtml(t.targetDate) : ""}</small></div><button type="button" class="text-button" data-task-id="${t.id}">${t.done ? "Reabrir" : "Hecho"}</button></div>`).join("");
...
function renderTransactions() {
  ...
  if (!list) return;
  if (!state.transactions.length) { list.className = "transaction-list empty"; list.textContent = "Sin movimientos todavía."; return; }
  list.className = "transaction-list";
  list.innerHTML = state.transactions.slice().reverse().map((t) => {
    ...
    return `<div class="transaction-row"><div><strong>${escapeHtml(t.description)}</strong><small>${typeLabel} · ${escapeHtml(t.category)} · ${method}${new Date(t.date).toLocaleDateString("es-MX")}</small></div><div class="${amountClass}">${sign}${money(t.amount)}</div></div>`;
  }).join("");
}
...
function render() {
  ...
  renderTasks();
  renderTransactions();
  if (exists("githubToken") && getGithubToken() && !$("githubToken").value) { $("githubToken").placeholder = "Token guardado en este navegador"; setSyncStatus("Sync activo"); }
...
function bindEvents() {
  ...
  $("saveGithubConfig")?.addEventListener("click", () => {
    ...
    if (!token) { alert("Pega primero tu GitHub token."); return; }
  ...
  $("saveGoogleConfig")?.addEventListener("click", () => {
    ...
    if (!url) { alert("Pega primero tu Google Apps Script Web App URL."); return; }
  ...
  $("saveOpenAiConfig")?.addEventListener("click", () => {
    ...
    if (!url) { alert("Pega primero tu OpenAI proxy URL."); return; }
  ...
  document.querySelectorAll('input[name="type"]').forEach((input) => {
    input.addEventListener("change", () => { setActiveQuickAction(input.value); togglePaymentMethod(); });
  });
}
